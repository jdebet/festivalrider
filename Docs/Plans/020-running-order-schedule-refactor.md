...
- `Services/BandServiceTests.cs` — extend with `UpdateShow` test asserting every new scalar copies across and nested `Bands` / `RunningOrders` references stay intact. Add `DateOfOpening`-shift test asserting show meal slots, RO meal-hour overrides, and every `Slot.CateringSlot` shift by the day delta and preserve time-of-day. Add `UpdateRunningOrder` `ShowDayNumber`-shift test for RO meal overrides and slot catering. Add `DeleteStage` test asserting orphaned `StageLinkGroup.StageIds` are pruned and groups with `< 2` remaining ids are dropped.
